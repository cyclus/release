...
cmake `pwd`  -DCMAKE_EXE_LINKER_FLAGS="-L/`pwd`/../install/lib -lcoinblas -lcoinlapack -L/`pwd`/../install/lib/libboost_filesystem.so.1.50.0" -DCMAKE_INSTALL_PREFIX=`pwd`/../install -DCOIN_ROOT_DIR=`pwd`/../install -DBOOST_ROOT=`pwd`/../install -DBoost_NO_SYSTEM_PATHS=ON
make
make install
cd ..
